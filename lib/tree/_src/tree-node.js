...
export default {
  ...
  methods: {
    beforeEnter (el) {
      el.setAttribute('style', `height: 0px; ${this.transition.join(' ')}`)
    },
    ...
    beforeLeave (el) {
      el.setAttribute('style', `height: ${el.offsetHeight}px; ${this.transition.join(' ')}`)
    },
    ...
    itemChecked () {
      ...
      this.treeInstance.onChecked(selected, this.nodeKey, this.nodeData)
    },
