export default {
   ...
   * @param {string|Array}[required] path the path of the property to get
   * @param {any} resolve the value returned for undefined resolved value.
   */
  getValueByPath (object, path, resolve) {
    path = path.match(/\w+/g) || []
    let len = path.length
    let index = 0

    while (index < len && object !== null && object !== undefined) {
      object = object[path[index++]]
    }

    object = index && index === len ? object : undefined

    return (object === null || object === undefined) && resolve !== undefined ? resolve : object
  },
