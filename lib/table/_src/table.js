...
export default {
  ...
  computed: {
    ...
    footerConfig () {
      if (helpers.typeof(this.summaryMethod) === 'function') {
        return this.summaryMethod(this.columnsConfig, this.data, this.summaryText, helpers.getValueByPath)
      }
  ...
  render (h) {
    ...
    const renderTh = (column, columnIndex) => {
      ...
      return h('th', {
        class: [
          ...
          this.cellClassName ? this.cellClassName({
            type: 'th',
            rowIndex: 0,
    ...
    const renderTd = (column, columnIndex, row) => {
      const _slot = this.slots[column.slotIndex]
    ...
    const renderColumns = (tag, row) => {
      return this.columnsConfig.map((col, index) => {
        ...
        if (tag === 'th') return renderTh(col, index)
        if (tag === 'td') return renderTd(col, index, row)
