  ...
  install: function (Vue) {
    ...
    Vue.directive('loading', {
      inserted: function (el, binding, vnode) {
        ...
        const { type, block, text } = el.dataset
        const _instance = Vue.prototype.$_createLoading(helpers.clearEmpty({
          parent: el,
          type: type,
          block: block ? /(block|true)/.test(block) : null,
          text: text
        })).show()

        vnode._loadingInstance = _instance
      },
